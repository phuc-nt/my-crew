...
emits a gateway action — `my_crew/actions/*` is untouched. The wake itself enqueues one
...
from my_crew.runtime.watcher_normalize import normalize_and_hash
...
def _poll_source(watcher: dict, loaded: Any, settings: Any) -> Any:
    ...
    if source == "jira":
        from my_crew.tools.jira_read import get_open_issues

        return get_open_issues(target, config=loaded.config)
    if source == "github":
        from my_crew.tools.github_read import get_open_prs
