"""Per-agent worker entrypoint (v2 M1-P3).
    ...
    python -m my_crew.runtime.worker --agent-id <id> --report <daily|weekly|okr|resource>
                                 [--audience internal|external] [--dry-run]
    python -m my_crew.runtime.worker --agent-id <id> --resume
...
from my_crew.profile.loader import LoadedProfile, load_profile
...
def _run_with_mcp_pool(fn: Callable[[], dict]) -> dict:
    ...
    """
    from my_crew.adapters.mcp_session_pool import McpSessionPool, _current_pool
...
def main(argv: list[str] | None = None, *, run_report: RunReport = _default_run_report) -> int:
    ...
    if not agent_id:
        print(
            "usage: python -m my_crew.runtime.worker --agent-id <id> --report <kind>",
            file=sys.stderr,
        )
        return 2
    ...
    # v65: per-minute timed-reminder delivery — NO-LLM (read due rows → telegram_send
    # via the gateway → mark sent). Branches before graph dispatch like the ticks above.
    if kind == "reminder-sweep":
        from my_crew.runtime.reminder_sweep import run_reminder_sweep

        try:
            result = run_reminder_sweep(loaded, settings)
        except Exception as exc:  # noqa: BLE001 — record the failure, never crash
            logger.exception("worker %s/reminder-sweep failed", agent_id)
            append_run_event(data_dir, _event(agent_id, kind, "internal", "error", None, False))
            print(f"error: {exc}", file=sys.stderr)
            return 1
        append_run_event(
        ...
        logger.info("worker %s reminder-sweep: %s (checked=%s)", agent_id,
    ...
    if audience == "internal" and result.get("report_text"):
        from my_crew.runtime.report_summary import summarize_report
