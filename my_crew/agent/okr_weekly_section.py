...
from my_crew.agent.okr_analyzer import OkrRollup, build_objectives
...
def _split_keys(cell: str) -> tuple[str, ...]:
    ...
    from my_crew.tools.confluence_read import parse_epic_keys
...
def build_okr_rollup(config: ReportingConfig) -> OkrRollup:
    ...
    from my_crew.tools.confluence_read import get_page_content, parse_okr_table
...
def weekly_okr_section(report_date: str, config: ReportingConfig) -> str:
    ...
    because OKR failed.
    """
    from my_crew.llm.okr_report_prompt import render_okr_table_xhtml
...
def weekly_okr_slack_line(config: ReportingConfig) -> str:
    ...
    from my_crew.llm.okr_report_prompt import overall_pct
