...
def discover_domains() -> tuple[str, ...]:
    ...
    `<x>-pack/` folder — NO core edit (this is what makes M6's `git diff my_crew/` = empty
...
def _pack_package_name(domain: str) -> str:
    ...
    needed (its builders call my_crew.* only) but a self-contained pack like HR does.
    """
...
def _load_pack_module(domain: str, module_name: str) -> ModuleType:
    ...
    submodule normally. The module can `import my_crew.*` (repo root on sys.path) and
...
class Pack:
    ...
    # interface lands in `my_crew/packs/tool_provider.py` and S3 binds the PM provider.
    tools: Any | None = None
