...
await p.goto('http://localhost:8799/chat', { waitUntil: 'networkidle' })
await p.locator('.office-composer input, .ops-composer input').first()
  .waitFor({ state: 'visible', timeout: 30000 })

await p.locator('.office-composer input, .ops-composer input').first()
  .fill('Viết 3 câu giới thiệu công ty Cold Start Co.')
await p.locator('button:has-text("Giao việc")').first().click()
console.log('submitted, waiting for the plan preview (real LLM call)...')
await p.waitForTimeout(45000)
await p.screenshot({ path: `${SHOT}/cs2-assign-result.png`, fullPage: true })
console.log('TEXT >>>', (await p.locator('body').innerText()).replace(/\s+/g,' ').slice(0, 1600))
