...
## Mở / sang sau
- ~~E2E live~~ **UAT SỐNG XONG 2026-07-13** (report trong plans/…/reports/uat-260713-1054): kill -9 giữa step → resume attempt mới + GC thread ✅; per-criterion review 4/4 và 5/5 ✅; fan-out mint→2 sub song song (2 pid OS cùng lúc)→gate→gather-copy→task done ($0.0099) ✅; review-ladder 3 vòng rớt → stall chờ CEO (đúng thiết kế) ✅. UAT lộ 2 finding: (1) LLM bọc JSON trong fence → vá `strip_json_fences` cho cả 4 parser (commit c04a5e0); (2) watcher v31 tạo task với plan_hash="watch-…" → task watcher không bao giờ dispatch được — **ĐÃ VÁ**: `_wake_plan_hash` tính `decomposition_content_hash` thật (khớp recompute của ticker), task watcher cũ trong prod cũng đã un-stall.
