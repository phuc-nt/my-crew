...
### US-4.1 — Duyệt việc gửi ra ngoài (v30: autonomy-first)
> **Là** CEO, **tôi muốn** có quyền kiểm soát (hoặc tự chủ nhanh), **để** cân bằng tốc độ + an toàn.

**v30 mặc định (autonomous):** việc đăng Slack / tạo Jira / gửi email **CHẠY NGAY** → audit ghi rationale "trust_mode=autonomous". Không chờ duyệt.

**Opt-in guarded:** ghim vào agent profile `safety.trust_mode: guarded` → việc gửi-ngoài **dừng chờ duyệt** (như cũ).

**Proposal automation:** đề xuất automation (không handler) **LUÔN chờ duyệt**, mọi mode.

**Tab Duyệt (cho guarded + proposal):**
- [ ] 🔗 Chỉ chứa việc từ **agent guarded** + **proposal**. Bấm **"Xem & duyệt"** → hộp thoại tóm tắt; gửi RA NGOÀI tô **đỏ đậm**. **Duyệt & thực hiện** / **Từ chối**.
- [ ] **Chat flatten (v30):** autonomous mode: bất kỳ thành viên Slack/Telegram allowlist kích hoạt → chạy ngay (không duyệt). Thu hẹp: ghim agent `trust_mode: guarded` hoặc thu hẹp allowlist.
...
### US-4.3 — Trust mode: từ autonomous → guarded (v30)
> **Là** CEO, **tôi muốn** kiểm soát mức độ tự chủ per-agent, **để** cân bằng rủi ro.

- [ ] **Mặc định:** agent chạy việc gửi-ngoài **ngay** (autonomous). Xem audit → rationale "trust_mode=autonomous".
- [ ] **Thu hẹp scope:** Đội → chọn agent → cài đặt → thêm dòng `safety.trust_mode: guarded` → agent chuyển thành chế độ kiểm soát (việc gửi-ngoài chờ duyệt trước).
- [ ] **Fleet flip khi upgrade:** agent cũ không khai trust_mode → thành autonomous sau v30 (release note báo rõ; admin/hr dry_run:false sẽ gửi email/Jira thật ngay).
