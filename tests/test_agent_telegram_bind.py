...
from my_crew.server import routes_agent_telegram
...
def agent_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("my_crew.server.routes_agent_telegram.REPO_ROOT", tmp_path, raising=False)
    ...
    monkeypatch.setattr("my_crew.server.profile_editor.save_profile_yaml", _save)
...
def test_bind_rejects_bad_token(agent_env, monkeypatch):
    ...
    monkeypatch.setattr("my_crew.actions.telegram_write.api_call", _fail)
...
def test_bind_without_chat_id_400_before_any_write(agent_env, monkeypatch):
    ...
    monkeypatch.setattr("my_crew.actions.telegram_write.api_call", _guard_getme)
